...
// I/O operators
ostream& operator<<(ostream& out, Fraction& myFraction) {
    // if both are negative, then make them positive
    if (myFraction.numerator < 0 && myFraction.denominator < 0) { 
        myFraction.numerator = abs(myFraction.numerator);
        myFraction.denominator = abs(myFraction.denominator);
    }
...
istream& operator>>(istream& in, Fraction& myFraction) {
	...
	{
        // use greatest common divisor function to reduce fraction
        int tmp_gcd = gcd(abs(myFraction.numerator), abs(myFraction.denominator));
        myFraction.numerator /= tmp_gcd;
        myFraction.denominator /= tmp_gcd;
		return in;
	}
}
